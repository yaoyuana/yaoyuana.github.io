(function () {
  ...
  var LINES = {
    ...
    blankStreak: ['你连续点，我当跑步机了。'],
    poke: ['别点我脸上，痒。', '点我头也没有菜单。', '我不是搜索框。'],
    link: ['那个能点，我不挡。', '去友邻的话带我？……也行，你去吧。'],
    self: [
  ...
  function onClick(event) {
    ...
    if (isInteractive(event.target)) {
      blankStreak = 0;
      speak(pick(LINES.link.concat(LINES.self.slice(1))), { force: true });
      return;
    }
    if (nearDog(event)) {
      blankStreak = 0;
      speak(pick(LINES.poke.concat(LINES.self.slice(0, 1))), { force: true });
      return;
    }
    blankStreak += 1;
  ...
  function bumpIdle() {
    ...
    idleTimer = setTimeout(function () {
      if (said.idle || document.visibilityState !== 'visible') return;
      said.idle = true;
